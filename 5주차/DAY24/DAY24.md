...
<br/>
이러한 툴들을 이용해서 그래프를 분석할 수 있을까? 그래프를 벡터로 나타낼 수 있으면 가능 할것이다.    
<br/>
 이번 강의에서는 그래프의 정점(Node)을 벡터로 표현하는 방법인 정점 임베딩(Node Embedding)에 대해 배운다.   

정점을 어떻게 벡터로 표현하는지, 정점사이의 유사성을 어떻게 표현하는지에 집중하여 강의를 들어보자.
...
- 따라서 많은 경우 근사식을 사용한다.
- 모든 정점에 대해서 정규화 하는 대신 **몇 개의 정점을 뽑아서 비교 하는 형태**이다. 이때 뽑힌 정점들을 `네거티브 샘플`이라고 부른다
...
- 사용자별 영화 평점 데이터가 사용
![넷플릭스](넷플릭스.PNG)
- 평균 제곱근 오차 0.9514을 0.8564까지 낮출 경우 100만불의 상금을 받는 조건
- 넷플릭스 챌린지를 통해 추천시스템의 성능이 비약적으로 발전

![넷플릭스2](넷플릭스2.PNG)

- **훈련 데이터(Training Data)** : **48만명 사용자**의 **1만 8천개의 영화**에 대한 **1억 개의 평점**(2000~2005년)

- **평가 데이터(Test Data)** :  각 사용자의 **최신 평점 280만개**로 구성

## 기본 잠재 인수 모형
- 넷플 챌린지에서 가장 비약적으로 발전, 현재 까지도 매우 잘 활용됨

### 잠재 인수 모형 개요

- 잠재 인수 모형(Latent factor Model)의 핵심 : 사용자와 상품을 벡터로 표현하는 것
- 사용자와 영화를 임베딩한 예시 
![잠재인수](잠재인수.PNG)

### 손실함수

- 사용자와 상품의 임베딩의 내적(Inner Product)이 평점과 최대한 유사 하도록 하는 것입니다.
...
- 잠재 인수 모형은 다음 손실 함수를 최소화 하는 P와 Q를 찾는것을 목표로 한다.
...
- 과적합을 방지하기 위하여 정규화 항을 손실함수에 더해 준다.
![손실함수4](손실함수4.PNG)
...
- 구체적으로 사용자 편향과 상품 편향을 시간에 따른 함수로 가정 한다.
...
### 실습
---
- https://colab.research.google.com/drive/1I7WdiOUOGKfdH6WWA1vtLPQt78KMPQQJ?usp=sharing

- https://colab.research.google.com/drive/1ZlsS1S-0TGxOTrz6x83JpcpwbG0ZoDmH?usp=sharing